...
internal sealed class SlotOverlapGuard(IApplicationDbContext dbContext)
{
    ...
    /// of the same level. <paramref name="excludedSlotId"/> lets an update ignore the slot it is
    /// moving; pass null when creating.
    /// </summary>
    public async Task<Result> EnsureNoOverlapAsync(
        int stageId, int periodNumber, DateOnly start, DateOnly end, int? excludedSlotId, CancellationToken ct)
    {
