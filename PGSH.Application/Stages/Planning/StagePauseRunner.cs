...
/// window of periods. Mirrors <see cref="StagePeriodRunner"/>: load every in-scope assignment
/// once, mutate the relevant <see cref="ServicePeriod"/>s, persist once. Idempotent — re-running
/// on an already-paused/resumed selection finds nothing pending.
/// </summary>
internal sealed class StagePauseRunner(IApplicationDbContext dbContext)
{
    public Task<Result<int>> PauseStageAsync(
        ...
        IReadOnlyList<int>? periodNumbers, PauseKind kind, string? reason, CancellationToken ct) =>
