...
> | **0bv** | **La garde `SingleService` sur les cellules publiées, avant qu'un stage `SingleService` soit publié.** `UpdateStageSlotCommandHandler` n'a **aucune** garde de publication et réécrit une fenêtre sans toucher aux périodes publiées depuis elle ; `SetCohortSlotAssignment` refuse sur « la cohorte est publiée » là où `PublishedCells.IsCellPublishedAsync` répondrait « *cette* cellule est-elle publiée ». | ⚠ **Latente uniquement par accident de données** : les huit stages publiés de la 3ᵉ MED sont tous `PerPeriod`. Sous `SingleService` les *k*ₛ cellules d'un séjour se replient en une période, et seule la première porte le lien — donc les cellules suivantes se lisent comme libres et seraient réécrites sous un stage déjà commencé. `docs/planning-rotation.md`, section `PublishedCells`. |
