...
**Neuvième et dernière pièce : l'écran** — `AxisRelayPage`, dépôt `PGSH_Frontend` (Formation →
« Recalcul de l'axe »). Aperçu, les deux comptes confirmés séparément, `SafePointBanner`, et le
tableau avant/après. Piloté de bout en bout sur la vraie 4ᵉ MED.

⚠ **Trois conventions de `PGSH.Frontend/CLAUDE.md` étaient enfreintes par la première version**, et
aucune n'était visible au type-check : §1e (un `detailOf` maison au lieu de `problemMessage`), §1i
(la requête étant *lazy*, l'aperçu survivait à un changement de promotion et décrivait l'ancienne
sous le nom de la nouvelle), §1g-bis (pas de `SafePointBanner` sur un acte qui réécrit 4 625
rotations — le document le nomme pourtant parmi les trois). Corrigées et vérifiées à l'écran.

⚠ **Et le panneau des suspensions pointe désormais vers lui** (`?levelId=`). Il comptait déjà les
colonnes traversées en disant que ces jours étaient perdus faute de déplacement : exact, et sans
issue. C'est le reproche que ce dépôt fait à ses propres rapports, pris par l'autre bout — le remède
existait, l'écran n'y menait pas. Nouvelle règle §1m côté client.
