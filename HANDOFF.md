...
> | **0bt** | ⚠ **Nettoyer le pied du test §58, resté dans la base** — la pile s'est arrêtée pendant le test (exception de routage, débogueur en pause). Dans cet ordre : `DELETE /api/students/dcf7cd8d-1a5c-4de6-b0f8-da1dce664270`, `DELETE /api/students/efc7d0d8-d8ea-4077-9868-e20dddebe06c`, `DELETE /api/cohorts/19935`, `DELETE /api/stages/21/slots/779` (le créneau **et** sa cellule 10796), `DELETE /api/groups/5095`. | ⚠ **Le créneau 779 est le seul élément partagé** : un `StageSlot` est keyed (stage, année, période) et appartient à toute la promotion, pas au roster de test. La 4ᵉ Pharmacie n'en avait **aucun** avant (0 `StageSlots` dans toute la base), donc il est purement additif — mais il est visible sur la grille de la promotion tant qu'il est là. ⚠ **Ce que le nettoyage ne retire pas** : l'enregistrement `AffectationImport` (marqué `Reversed`), qui restera en référençant des étudiants supprimés. C'est cohérent avec la conception — un acte se garde — mais c'est de la litière de test, et rien n'expose de suppression. À trancher : faut-il un élagage des imports dont plus aucune inscription n'existe ? |
