...
> | 0e | **Audit roster creation, rotation-group assignment and « vider les groupes ».** | `PARTITIONS_CLEARED` is recorded and the acts that *create* a cut are not, so « qui a découpé cette promotion, et quand ? » is unanswerable — which is exactly the question the empty 2026-2027 rosters raised, and it could not be answered. The destructive act leaves a trace and the constructive one does not: the wrong way round. |
...
>
> | 1d | **`POST stages/revalidate` has no frontend caller at all.** | Found 2026-09-01 while writing §34: the endpoint, the command, the guards and the tests all exist, and **no component in `PGSH.Frontend/src` calls it**. So the one act that lets a student make good a failed stage is reachable only through Scalar or curl — by an admin who knows the registration id, the stage id and a cohort id. §34 part C is written against Scalar for that reason. Separate from item 13, which is about the command's *scope*; this is that it has no door. |
> | 1e | **Nothing applies a stage's duration — neither the catalogue's nor the text's.** | Measured 2026-09-01: no code on the revalidation / dossier / progression / export path reads `DurationInDays` at all. `RevalidateStageCommand` takes the window the operator types. So for the 92 6ᵉ année students who owe MED3 Chirurgie under 2174.18, the 66 j.o. that text states is **recorded** and now **visible** (§33) but **applied nowhere** — the evidence being that the one such window on record ran 65 j.o., matching 66 and not the catalogue's 30. The fix is not to make the catalogue authoritative but to resolve the duration through `r.CnpnVersionId ?? r.Student.CnpnVersionId` where a window is proposed. Ties into `PHASES.md` §15.1. |
>
